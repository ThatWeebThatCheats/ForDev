...
bool ImGui::BeginAndroid(const char* name, bool* p_open, bool mainframe, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (mainframe)
			{
				window->DrawList->AddRectFilled(window->Pos - ImVec2(0, -600), window->Pos + ImVec2(window->Size.x - 2, window->Size.y + 1), ImColor(0.15625f, 0.15625f, 0.15625f, 1.00f), window_rounding);
...
{
	PushStyleColor(ImGuiCol_Text, selected ? ImVec4(0.00f, 0.90f, 0.00f, 1.00f) : ImVec4(1.f, 1.00f, 1.00f, 1.00f));
...
bool ImGui::MyButtonEx(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if (inuse)
	{
		...
		// lines
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Min.y + 1), ImVec2(bb.Max.x, bb.Min.y), GetColorU32(ImVec4(0.00f, 0.9f, 0.0f, 1.00f))); // ImVec4(1.07f, 1.07f, 1.07f, 1.00f)
		RenderFrame(ImVec2(bb.Min.x - 25, bb.Max.y - 1), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(ImVec4(0.00f, 0.9f, 0.0f, 1.00f)));
	}
	else if (!inuse)
		RenderFrame(ImVec2(bb.Max.x - 1, bb.Min.y - 4), ImVec2(bb.Max.x, bb.Max.y + 4), GetColorU32(ImVec4(0.00f, 0.90f, 0.0f, 1.00f)));
	// ICON TEXT
	if (icon)
	{
		// icon
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, const char* display_format, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 3), grab_bb.Max - ImVec2(0, 3), ImColor(0, 230, 0), ImColor(0, 230, 0), ImColor(0, 230, 0), ImColor(0, 230, 0));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
	{
		...
		window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1) - ImVec2(20, 1), ImColor(0, 255, 0), ImColor(0, 255, 0), ImColor(0, 255, 0), ImColor(0, 255, 0));
	}
	else
	{
		window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max - ImVec2(20, 1), ImColor(0.1f, 0.1f, 0.1f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (/*hovered || */selected)
	{
		...
		window->DrawList->AddRectFilledMultiColor(bb_with_spacing.Min, bb_with_spacing.Max, ImColor(0, 255, 0), ImColor(0, 255, 0), ImColor(0, 255, 0), ImColor(0, 255, 0));
